...
INSETproc1 proc 
	...
	mov objZaddr, rcx ;objZaddr <- z->this
	mov objCaddr, rdx ;objZaddr <- c->this
	...
	BeginingOfCalculatonLoop:
		;---------------------------------------------------
		;z = z*z = (z.real+i*z.img)*(z.real+i*z.img)=z.real*zreal + i*2*z.real*z.img -z.img*z.img =
		;= (z.real*z.real - z.img*z.img) + i(2*z.real*z.img)
		...
		fstp REAL8 ptr [three]	;pop
		fstp REAL8 ptr [dist]	;pop
		
		and eax, 0100011100000000B
		cmp eax, 0000000000000000B
		jg	breakingLoop	;jump if st0 > st1		
	...
	mov rbx, zReal 
	mov qword ptr [rax], rbx  ;zReal -> z.real 
	
	mov rbx, zImg
	mov qword ptr [rax+8], rbx ;zImg -> z.real 
